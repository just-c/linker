...
static unsigned long sec_vaddr[3] = {
    0x800000}; /* virtual address of sections */
...
static Elf_Sym *obj_find(struct obj *obj, char *name) {
  ...
  for (i = 0; i < obj->nsyms; i++) {
    ...
    if (ELF_ST_BIND(sym->st_info) == STB_LOCAL || sym->st_shndx == SHN_UNDEF)
      continue;
    if (!strcmp(name, obj->symstr + sym->st_name)) return sym;
  }
  return NULL;
}
...
static void obj_init(struct obj *obj, char *mem) {
  ...
  for (i = 0; i < obj->ehdr->e_shnum; i++) {
    if (obj->shdr[i].sh_type != SHT_SYMTAB) continue;
...
static struct secmap *outelf_mapping(struct outelf *oe, Elf_Shdr *shdr) {
  ...
  for (i = 0; i < oe->nsecs; i++)
    if (oe->secs[i].o_shdr == shdr) return &oe->secs[i];
  return NULL;
}

static int outelf_find(struct outelf *oe, char *name, struct obj **sym_obj,
                       Elf_Sym **sym_sym) {
  int i;
...
static unsigned long bss_addr(struct outelf *oe, Elf_Sym *sym) {
  ...
  for (i = 0; i < oe->nbss_syms; i++)
    if (oe->bss_syms[i].sym == sym) return oe->bss_vaddr + oe->bss_syms[i].off;
  return 0;
}
  ...
  switch (ELF_ST_TYPE(sym->st_info)) {
    ...
    case STT_FUNC:
      if (name && *name && sym->st_shndx == SHN_UNDEF)
        if (outelf_find(oe, name, &obj, &sym)) die_undef(name);
      if (sym->st_shndx == SHN_COMMON) return bss_addr(oe, sym);
      s_idx = sym->st_shndx;
      s_off = sym->st_value;
...
static void outelf_reloc(struct outelf *oe) {
  ...
  for (i = 0; i < oe->nobjs; i++) {
    ...
    for (j = 0; j < obj->ehdr->e_shnum; j++)
      if (obj->shdr[j].sh_type == SHT_REL_) outelf_reloc_sec(oe, i, j);
  }
}
...
static void outelf_bss(struct outelf *oe) {
  ...
  for (i = 0; i < oe->nobjs; i++) {
    ...
    for (j = 0; j < obj->nsyms; j++)
      if (obj->syms[j].st_shndx == SHN_COMMON) alloc_bss(oe, &obj->syms[j]);
  }
}
...
static void build_symtab(struct outelf *oe) {
  ...
  for (i = 0; i < oe->nobjs; i++) {
    ...
    for (j = 0; j < obj->nsyms; j++) {
      ...
      if (!*name || bind == STB_LOCAL || sym->st_shndx == SHN_UNDEF) continue;
...
static void outelf_write(struct outelf *oe, int fd) {
  int i;
  oe->ehdr.e_entry = outelf_addr(oe, entry) - sec_vaddr[I_CS] + sec_laddr[I_CS];
  if (!nosyms) build_symtab(oe);
  ...
  for (i = 0; i < oe->nsecs; i++) {
    ...
    if (SEC_BSS(sec->o_shdr)) continue;
...
static void outelf_add(struct outelf *oe, char *mem) {
  ...
  struct obj *obj;
  int i;
  if (ehdr->e_type != ET_REL) return;
  e_machine = ehdr->e_machine;
  e_flags = ehdr->e_flags;
  if (oe->nobjs >= MAXOBJS) die("neatld: MAXOBJS reached!");
  obj = &oe->objs[oe->nobjs++];
  obj_init(obj, mem);
  for (i = 0; i < ehdr->e_shnum; i++) {
    struct secmap *sec;
    if (!(shdr[i].sh_flags & 0x7)) continue;
    if (oe->nsecs >= MAXSECS) die("neatld: MAXSECS reached!");
...
static int link_cs(struct outelf *oe, Elf_Phdr *phdr, unsigned long faddr,
  ...
  for (i = 0; i < oe->nsecs; i++) {
    ...
    if (!SEC_CODE(sec->o_shdr)) continue;
...
static int link_ds(struct outelf *oe, Elf_Phdr *phdr, unsigned long faddr,
  ...
  for (i = 0; i < oe->nsecs; i++) {
    struct secmap *sec = &oe->secs[i];
    if (!SEC_DATA(sec->o_shdr)) continue;
    sec->vaddr = vaddr + len;
    sec->faddr = faddr + len;
    len += sec->o_shdr->sh_size;
  }
...
static int link_bss(struct outelf *oe, Elf_Phdr *phdr, unsigned long faddr,
...
static int sym_undef(struct outelf *oe, char *name) {
  ...
  for (i = 0; i < oe->nobjs; i++) {
    ...
    for (j = 0; j < obj->nsyms; j++) {
      ...
      if (strcmp(name, obj->symstr + sym->st_name)) continue;
...
static int outelf_ar_link(struct outelf *oe, char *ar, int base) {
  ...
  for (i = 0; i < nsyms; i++) {
    int off = get_be32((void *)ar_index + i * 4) + sizeof(struct arhdr);
...
static void outelf_archive(struct outelf *oe, char *ar) {
  ...
  for (;;) {
    ...
    if (name[0] == '/' && name[1] == '/' && name[2] == ' ') outelf_add(oe, ar);
    ar += size;
  }
}
...
static char *obj_add(struct outelf *oe, char *path) {
  ...
  if (!buf) die("neatld: cannot open object!");
  if (is_ar(path))
...
int main(int argc, char **argv) {
  ...
  int fd;
  int i = 0;
  if (argc < 2) die("neatld: no object given!");
  outelf_init(&oe);
  ...
  while (++i < argc) {
    ...
    if (argv[i][1] == 'l') {
      char path[PATHLEN];
      if (lib_find(path, argv[i] + 2)) die("neatld: cannot find library!");
  ...
  if (fd < 0) die("neatld: failed to create the output!");
  outelf_write(&oe, fd);
  close(fd);
  for (i = 0; i < nmem; i++) free(mem[i]);
  return 0;
}
